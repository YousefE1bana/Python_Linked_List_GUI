...
class LinkList:
    def __init__(self,root):
        ...
        #Make the Canvas
        self.canvas_make = Canvas(self.window,bg="#301934",width=1350,height=500,relief=RAISED, bd=8)
    ...
    def heading_with_label_subheading(self):
        self.head_name = Label(self.window,text="Linked List",font=("Arial",35,"bold","italic"),
                               bg="#301934",fg="yellow")
        self.head_name.place(x=520,y=25)

        self.information =  Label(self.window,text="Start is a pointer that pointing the first node",font=("Arial",20,"bold","italic"),
                               bg="#301934",fg="yellow")
    ...
    def make_start_with_other(self):
        ...
        self.start_label = Label(self.canvas_make,text="Head",font=("Arial",15,"bold"),bg="#301934",
                                 fg="green")
        ...
        self.start_initial_point_null = Label(self.canvas_make, text="NULL", font=("Arial", 15, "bold"), bg="#301934",
                                 fg="green")
        ...
        self.temp_label = Label(self.canvas_make,text="temp",font=("Arial",15,"bold"),bg="#301934",
                                 fg="green")

        self.temp1_label = Label(self.canvas_make, text="temp1", font=("Arial", 15, "bold"), bg="#301934",
                                fg="green")
    ...
    def make_btn(self):
        ...
        self.delete_particular_node.place(x=1090, y=540)
                # New Buttons for Average, Update, and Search
        self.average_btn = Button(self.window, text="Average", bg="black", fg="red",
                                 font=("Arial", 15, "bold"), relief=RAISED, bd=10,
                                 command=self.calculate_average)
        self.average_btn.place(x=20, y=700)  # Adjust placement as needed

        self.update_btn = Button(self.window, text="Update Node", bg="black", fg="red",
                                font=("Arial", 15, "bold"), relief=RAISED, bd=10,
                                command=self.update_node_input)
        self.update_btn.place(x=220, y=700)  # Adjust placement as needed

        self.search_btn = Button(self.window, text="Search Node", bg="black", fg="red",
                                font=("Arial", 15, "bold"), relief=RAISED, bd=10,
                                command=self.search_node_input)
        self.search_btn.place(x=420, y=700)  # Adjust placement as needed
    ...
    def set_of_input_method(self):
        ...
        self.position_label = Label(self.window,text="Enter the node position after you want to insert new node",font=("Arial",13,"bold"),bg="#E6E6FA",
                                 fg="brown")
    ...
    def make_node_with_label(self,take_notation):
        ...
        #New_node label make
        self.new_node_label = Label(self.canvas_make,text="New node",font=("Arial",10,"bold"),bg="#301934",
                                 fg="yellow")
        ...
        self.data_label = Label(self.canvas_make,text="Data",font=("Arial",10,"bold"),bg="#301934",
                                 fg="yellow")
        ...
        self.next_label = Label(self.canvas_make, text="Next", font=("Arial", 10, "bold"), bg="#301934",
                                fg="yellow")
    ...
    def input_take(self,take_notation):
        ...
        self.element_take_label = Label(self.window, text="Enter the element value",
                                        bg="#E6E6FA", fg="brown", font=("Arial", 12, "bold"))
    ...
    def make_main_container_with_node_value_set_and_next_arrow_creation(self,take_notation):
        ...
        # take notation=0 means insert_at_last perform...
        # take notation=1 means insert_at_first perform...
        # take notation=2 means insert_after_node perform...
        # take notation=3 means delete_particular_node perform
    def calculate_average(self):
        if not self.node_value_store:
            messagebox.showinfo("Average", "List is empty, cannot calculate average.")
            return

        try:
            numeric_values = [int(val) for val in self.node_value_store]  # Convert to numbers
            average = sum(numeric_values) / len(numeric_values)
            messagebox.showinfo("Average", f"Average of node values: {average}")
        except ValueError:
            messagebox.showerror("Error", "List contains non-numeric values. Cannot calculate average.")

    def update_node_input(self):
        self.input_take(4)  # Use a new notation for update input
        self.element_take_label.config(text="Enter position to update:")
        self.add_btn.config(command=self.update_node)

    def update_node(self):
        try:
            position = int(self.value_entry.get())
            if 1 <= position <= len(self.node_value_store):
                self.input_take(5)
                self.element_take_label.config(text="Enter new value:")
                self.add_btn.config(command=lambda: self.perform_update(position))
    ...
    def perform_update(self, position):
      try:
        new_value = self.value_entry.get()
        self.node_value_store[position - 1] = new_value
        self.linked_list_data_next_store[position-1][0].config(text=new_value)
        self.reset_with_store(0)
        messagebox.showinfo("Success", "Node updated.")

      except ValueError:
        messagebox.showerror("Error", "Invalid input. Please enter a valid value.")

    def search_node_input(self):
        self.input_take(6) # Use a new notation for search input
        self.element_take_label.config(text="Enter value to search:")
        self.add_btn.config(command=self.search_node)

    def search_node(self):
        search_value = self.value_entry.get()
        try:
            search_value = int(search_value)
            if search_value in [int(i) for i in self.node_value_store]:
                position = [int(i) for i in self.node_value_store].index(search_value) + 1
                messagebox.showinfo("Found", f"Value {search_value} found at position {position}")
            else:
                messagebox.showinfo("Not Found", f"Value {search_value} not found")
        except ValueError:
            messagebox.showinfo("Not Found", f"Value {search_value} not found")
    ...
    def delete_last_node(self,locator):
        ...
        self.insert_after_node.config(state=NORMAL)
        self.delete_particular_node.config(state=NORMAL)
        self.average_btn.config(state=NORMAL)
    ...
    def delete_single_node_infrastructure(self):
        ...
        if len(self.node_value_store) == 0: 
           self.information.config(text="Nothing to delete list is empty")
           return
        else:
           self.information.config(text="First node position: 1")
    ...
    def delete_single_node(self):
        ...
        if int(self.delete_entry.get())>len(self.node_value_store) or int(self.delete_entry.get())<1:
           messagebox.showerror("Position not found")
        elif int(self.delete_entry.get()) == 1: 
